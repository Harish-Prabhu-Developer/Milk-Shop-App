    ...
    <Animated.View entering={FadeInUp.duration(500)}>
      ...
      >
        ...
        {Order.OrderStatus === 'Cancelled' && Order.CancelOrderDate && (
          <View className="mx-4 my-4 p-4 bg-red-50 rounded-2xl border border-red-200 shadow-sm">
            <View className="flex-row items-center mb-2">
              <MaterialIcons name="cancel" size={22} color="#DC2626" />
              <Text className="ml-2 text-lg font-bold text-red-700">
                Order Cancelled
              </Text>
            </View>
        ...
        <View className="mb-4 space-y-2 gap-4">
          ...
          <View className="flex-row justify-between items-center">
            <Text className="text-base text-gray-600">Received Status</Text>
            <Text
              className={`px-3 py-1 rounded-full text-sm font-semibold ${getStatusColor[Order.ReceivedStatus as keyof typeof getStatusColor]}`}
            >
