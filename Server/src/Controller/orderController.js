...
export const createOrder = async (req, res) => {
    try {
        ...
        res.status(201).json({ msg: "Order created successfully", order:fulneworder });
    } catch (error) {
...
export const ReOrder = async (req, res) => {
    try {
        ...
        res.status(201).json({ msg: "ReOrder placed successfully", order: fulneworder });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};
    ...
    try {
        ...
        if (Array.isArray(ReceivedItems)) {
            ReceivedItems.forEach(received => {
                const existing = order.ReceivedItems.find(item => item.id === received.id);
